...
def parse_arguments():
    ...
    parser.add_argument('--processes', '-p', type=int, help='Maximum number of processes. Default = 10', default=10)
...
def _run_feature(feature, tags=None, userdata=None):
    ...
    if not userdata:
        ...
        params = f'--tags={tags} -D {" -D ".join(userdata)} --no-capture'
    cmd = f'behave --no-skipped -f allure_behave.formatter:AllureFormatter -o allure-results {params} -i {feature}'
